...
def train_abdt(train, trainlabel):
    iotbinclf = AdaBoostClassifier()
    trainclf = iotbinclf.fit(train, trainlabel)
    return trainclf


def predicted_abdt(clf, test):
    predictions = clf.predict(test)
    acc = accuracy_score(testl, predictions)
    return acc


clf = train_abdt(train, partly_flipped)

acc_test_b = predicted_abdt(clf, test)
