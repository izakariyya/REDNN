...
partly_flipped = np.transpose(partly_flipped.reshape((partly_flipped.shape[0], 1)))
...
params_values_b, cost_history_b, accuracy_history_b, grad_a = fully_connected_nn.train(traind, partly_flipped, NN_ARCHITECTURE, N_EPOCHS, LR,128)
...
#epsis =[.01, .02, .03, .04, .05, .06, .07, .08, .09, .1]
#epsis = [.1, .2, .3, .4, .5, .6, .7, .8, .9, 1.0]
#acc_testbr = []
#pre_testbr = []      
#for epsi in epsis:
#    X_attack = advsry(testd, testlbl, epsi)
...
#    pre_testbr.append(pre_test_b_r)
#    print(f"Epsilon: {epsi}, adver. acc. base:{acc_test_b_r:.3f}, adver. prec. base:{pre_test_b_r:.3f} ")
...
params_values_batch, cost_history_batch, accuracy_history_batch, grado = train_batch(traind, partly_flipped, NN_ARCHITECTURE, N_EPOCHS, LR, 128, 4)
...
#    pre_testbatchr.append(pre_test_batch_r)
#    print(f"Epsilon: {epsi}, adver. acc. optim.:{acc_test_batch_r:.3f}, adv. pre. optim.:{pre_test_batch_r:.3f} ")
