...
def train_adab(train, trainlabel):
    iotbinclf = iotbinclf = AdaBoostClassifier()
    trainclf = iotbinclf.fit(train, trainlabel)
    return trainclf


def predicted_adab(clf, test):
...
clf = train_adab(train, trainlabel)
...
acc_test_b = predicted_adab(clf, test)
